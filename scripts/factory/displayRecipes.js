...
setTimeout(() => console.log(allRecipes), 100)
...
async function filterRecipes(event) {
    ...
    const searchedRecipe = event.target.value.toLowerCase().trim();
    
    const filteredRecipes = recipes.forEach(recipe => recipe.filter(el => el.name.toLowerCase().includes(searchedRecipe)))
